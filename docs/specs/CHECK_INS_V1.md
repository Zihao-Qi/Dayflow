...
| Habit identifiers or expected identifiers missing or not an array | 400 | `VALIDATION_ERROR` | `ids` / `expectedIds` |
| Habit identifiers contain duplicates | 400 | `VALIDATION_ERROR` | `ids` / `expectedIds` |
| More than 1,000 habit identifiers in reorder request | 400 | `VALIDATION_ERROR` | `ids` / `expectedIds` |
| Habit identifier in reorder array is invalid | 400 | `VALIDATION_ERROR` | `ids` / `expectedIds` |
| Habit order out of date, membership mismatch, or order conflict | 409 | `CONFLICT` | — |
...
- **Atomic Habit Reordering**: Named "Move up" and "Move down" accessible buttons on each row with disabled boundaries (`index === 0` for up, `index === total - 1` for down) and single in-flight mutation locking (`reorderPending`). Activating a move updates the contiguous 0..N-1 `sortOrder` via `PATCH /api/habits` with compare-and-swap verification (`expectedIds`). Focus stays on the moved row: if the activated direction becomes disabled at an edge (e.g. moving to top/bottom), focus shifts automatically to the opposite usable move button on the same row. A live announcement describes the move (`Moved "Name" to position X of Y. [Now first/last.]`). No speculative reorder plus rollback: the confirmed first-attempt order applies only if captured array identity and `todayKey` still own current state, followed immediately by trailing read refresh.
...
- **Uncertain Save & Reconcile Protocol**: Network mutations are serialized globally across the history surface: while any check-in save is in flight, conflicting surface actions (Header Refresh/Close, other row Save buttons, and reconciliation triggers) reflect active saving state with disabled controls and visible busy feedback (`Another save in progress…`), while preserving draft editing and date navigation. Within each record, an in-flight or uncertain attempt retains a stable mutation ID. If a save attempt remains uncertain due to connection failure or lost response, the retained intention payload and mutation ID are locked against new intention changes while permitting same-intention retries while writable. A dedicated "Check status" action with accessible name matching visible text verbatim per WCAG 2.5.3 (`Check status for <name> on <date>`) enables exact-date reconciliation via `GET /api/habits/[id]/check-in?date=YYYY-MM-DD`, accessible both within the window and after date expiry (accompanied by a truthful read-only notice banner). Confirmed writes update local history state, reconcile evidence, trigger shell refresh, and report refresh errors without falsifying write status.

## Resolved Policy Decisions and Scope

- **v1 Scope Boundaries**: Habit reordering (`sortOrder` manipulation) is implemented atomically via `PATCH /api/habits`. Historical backfill across the 8-day window (`today - 7` through `today`) is delivered via `HabitHistoryDialog` (`GET /api/habits/history` and `PUT /api/habits/[id]/check-in`), providing complete retro-logging and exact-date reconciliation.
