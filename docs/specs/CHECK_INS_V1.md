...
- **Uncertain Save & Reconcile Protocol**: Network mutations are serialized globally across the history surface; saving disables conflicting surface actions and reflects active save state. Within each record, an in-flight or uncertain attempt retains a stable mutation ID. If a save attempt remains uncertain due to connection failure or lost response, the retained intention payload and mutation ID are locked against new intention changes while permitting same-intention retries while writable. A dedicated "Check status" action enables exact-date reconciliation via `GET /api/habits/[id]/check-in?date=YYYY-MM-DD`, accessible both within the window and after date expiry. Confirmed writes update local history state, reconcile evidence, trigger shell refresh, and report refresh errors without falsifying write status.
