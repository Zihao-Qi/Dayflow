...
- **Q1 Pre-Creation Backfill Tension**: An unresolved scope and policy interpretation question exists between Section *Recording and Backfill* (which permits recording a Check-in for "today or for any of the seven preceding days") and Section *Consistency* (which states "neither can a day before the Habit existed" be counted or missed). Whether a habit created mid-week may record valid check-in rows for days prior to its `createdAt` remains an open interpretation question retained without unilateral resolution.
- **Q2 Clipped vs. Fixed Weekly Target Tension**: An unresolved scope and policy interpretation question exists between Section *Consistency* (which states that a `TIMES_PER_WEEK` habit "keeps its weekly goal for the whole period" without clipping to elapsed days) and the current summary calculation (`summarizeHabits`, which clamps targets to countable period days via `Math.min(targetPerWeek, countableDays)`). This contradictory wording between spec intent and calculation is retained unresolved for future decision.
