...
| `PATCH /api/tasks/:id` | Completed Project with unfinished Task | 409 | `{"error":"Reopen the completed project before adding unfinished work.","code":"RELATIONSHIP_CONFLICT","field":"projectId"}` | `code`, `field` | `tests/unit/task-route-contracts.test.ts` — “Task POST and PATCH pin each placement error independently” | New |
