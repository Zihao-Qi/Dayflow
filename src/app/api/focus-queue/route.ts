...
export async function POST(request: NextRequest) {
  try {
    ...
    const tasks = await runInTransaction((tx) => addToFocusQueue(tx, input.taskId, input.placement));
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const tasks = await runInTransaction((tx) => reorderFocusQueue(tx, input.ids, input.expectedIds));
    return NextResponse.json({ tasks });
  } catch (error) {
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const tasks = await runInTransaction((tx) => removeFromFocusQueue(tx, input.taskId));
    return NextResponse.json({ tasks });
  } catch (error) {
